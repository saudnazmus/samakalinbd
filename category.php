...
$getcategory = $_GET['category'];
	...
	if(isset($_GET['pageno']) && $_GET['pageno']){
		...
		if($pageno=='last'){
			...
			$cat_lid_sql = "SELECT * FROM articles WHERE category=$getcategory ORDER BY id DESC LIMIT $limit";
			...
			$sql_catpage_art = "SELECT articles.id, articles.head, images.imagename, articles.details, articles.category, articles.publishdate, articles.modifytime FROM `articles` INNER JOIN images ON articles.id=images.articleid WHERE articles.category='$getcategory' AND articles.status='1' AND articles.id<$cat_last_id ORDER BY articles.id DESC LIMIT 3";
			$sql_catpage_art_run = mysqli_query($conn, $sql_catpage_art);
		}

	}else{
		$sql_catpage_art = "SELECT articles.id, articles.head, images.imagename, articles.details, articles.category, articles.publishdate, articles.modifytime FROM `articles` INNER JOIN images ON articles.id=images.articleid WHERE articles.category='$getcategory' AND articles.status='1' ORDER BY articles.id DESC LIMIT 3";
		$sql_catpage_art_run = mysqli_query($conn, $sql_catpage_art);
	}
	

$sql_catpage_list_art = "SELECT articles.id, articles.head, images.imagename, articles.details, articles.category, articles.publishdate, articles.modifytime FROM `articles` INNER JOIN images ON articles.id=images.articleid WHERE articles.category='$getcategory' AND articles.status='1' ORDER BY articles.id DESC LIMIT 20";
$sql_catpage_list_art_run = mysqli_query($conn, $sql_catpage_list_art);

$sql_category_name = "SELECT `id`, `category_name` FROM `category` WHERE `id`=$getcategory ";
$sql_category_name_run = mysqli_query($conn, $sql_category_name);

$category_name = mysqli_fetch_assoc($sql_category_name_run);
$category_category = $category_name['category_name'];
...
<head>
		...
		<title><?php echo $category_category; ?> | সমকালীন বাংলাদেশ</title>
				...
				<div class="row">
					<div class="col-lg-8 mt-4">
						<?php
							...
							$catpage_art_category = $catpage_art['category'];
						...
						<div class="m-0 px-2 pb-2 my-0">
							...
							<span class="text-muted h6"> <?php echo $category_category; ?> </span>
						</div>
